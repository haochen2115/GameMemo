...
| 分支 | 内容 | 状态 |
|---|---|---|
| `main` | 个人记忆 P3：P2 + 概念标签、情景兜底 | 当前 SOTA（retrieval_v2 0.912；e2e_v5 test 0.825） |
...
1. `pytest` 全部通过。
2. 在 `bench/candidate.json` 声明的每个评测上（检索看 `bench/sota.json`，端到端看 `bench/sota_e2e.json`；目前分别是 `retrieval_v2` 和 `e2e_v5` 的 **test** split）：
