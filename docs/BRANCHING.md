...
| 分支 | 内容 | 状态 |
|---|---|---|
| `main` | v0：关键词匹配记忆（原始版本） | 当前 SOTA，等待被超越 |
| `archive/v0-keyword-baseline` | v0 的冻结快照（= main @ `3a866ba`） | 只读 |
| `research/personal-memory-foundation` | 个人记忆 v1：混合检索、写入链路重构、双时态更新、评测与门禁 | 开发中，未过门禁（见 BENCHMARK.md） |
| `research/shared-memory-quorum` | 多租户共享记忆（QUORUM / COMMONS） | 研究预览，尚无模型评测 |

## SOTA 门禁（进入 main 的条件）

1. `pytest` 全部通过。
2. 在 `bench/sota.json` 指定的数据集和 split（目前是 `retrieval_v1` 的 **test** split）上：
...
3. 用脚本检查，不靠人工判断：
   ...
   ```
   CI 会在所有指向 `main` 的 PR 上自动跑这一步（`.github/workflows/ci.yml` 的 `sota-gate`）。
...
- 在 GitHub Settings → Branches 给 `main` 加保护规则：必须通过 PR 合入，且 `ci / test` 和 `ci / sota-gate` 两个检查必须通过。
- 删除已改名的旧分支：`codex/share-memory`（已改名为 `research/shared-memory-quorum`，提交完全相同）。
